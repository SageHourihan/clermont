...
const FEED_ORDER = ['GEO', 'ENV', 'MKT', 'INF'];
...
function renderBar(label, count, total, fillClass) {
...
export function renderMetrics(containerId, events) {
    ...
    // Feed counts
    const byFeed = { GEO: 0, ENV: 0, MKT: 0, INF: 0 };
    for (const ev of events)
        byFeed[ev.feed]++;
    ...
    const priority = [...events]
        ...
        .slice(0, 5);
    const severityBars = SEVERITY_ORDER.map(sev => renderBar(sev, bySeverity[sev], total, `metrics__bar-fill--${sev.toLowerCase()}`)).join('');
    const feedBars = FEED_ORDER.map(feed => renderBar(feed, byFeed[feed], total, `metrics__bar-fill--${feed.toLowerCase()}`)).join('');
    el.innerHTML = `
