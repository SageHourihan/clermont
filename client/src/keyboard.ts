...
const FEED_CONTAINER_IDS: Record<Feed, string> = {
  GEO: 'geo-feed',
  ENV: 'env-feed',
  MKT: 'mkt-feed',
  INF: 'inf-feed',
...
function clearAllFocusStyles(): void {
...
function applyFocusToRow(panelIndex: number, rowIndex: number): void {
  clearAllFocusStyles()
...
function moveFocusRow(delta: number): void {
...
function moveFocusPanel(delta: number): void {
  state.panelIndex = (state.panelIndex + delta + PANEL_ORDER.length) % PANEL_ORDER.length
  state.rowIndex = 0
  applyFocusToRow(state.panelIndex, state.rowIndex)
}

function jumpToPanel(index: number): void {
  if (!state.active) enterNavMode()
  state.panelIndex = index
  state.rowIndex = 0
  applyFocusToRow(state.panelIndex, state.rowIndex)
}

function jumpToFirstRow(): void {
  state.rowIndex = 0
  applyFocusToRow(state.panelIndex, state.rowIndex)
}

function jumpToLastRow(): void {
  const feed = PANEL_ORDER[state.panelIndex]
  state.rowIndex = Math.max(0, state.events[feed].length - 1)
  applyFocusToRow(state.panelIndex, state.rowIndex)
}

function activateSelectedEvent(): void {
  const feed = PANEL_ORDER[state.panelIndex]
  ...
  document.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowRight':
        ...
        moveFocusPanel(+1)
        break
      case '1': jumpToPanel(0); break
      case '2': jumpToPanel(1); break
      case '3': jumpToPanel(2); break
      case '4': jumpToPanel(3); break
      case 'g':
